...
async function dbInsertPlay(play) {
  await fetch(`${SUPABASE_URL}/rest/v1/mm_plays`, {
    ...
    body: JSON.stringify({
      id: play.id,
      date: play.date,
      date_label: play.dateLabel,
      team_a: play.teamA,
      team_b: play.teamB,
      tourney: play.tourney,
      spread: play.spread,
      notes: play.notes,
      score: play.score,
      ...
      result: play.result,
      team_a_score: play.teamAScore,
      team_b_score: play.teamBScore,
      ...
      <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between" }}>
        ...
        <div style={{ display: "flex", gap: 4, alignItems: "center", marginLeft: 10, flexShrink: 0 }}>
          ...
          <button onClick={() => setExpanded(e => !e)} style={{
            ...
            background: expanded ? "#0f172a" : "#f1f5f9", color: expanded ? "#fff" : "#64748b", fontSize: 11,
          }}>{expanded ? "▲" : "▼"}</button>
        </div>
      </div>
  ...
  return (
    <div style={{ display: "grid", gridTemplateColumns: "1fr 285px", gap: 14, padding: "18px 20px" }}>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 10, position: "sticky", top: 75, alignSelf: "flex-start" }}>
        ...
        <button onClick={handleLog} disabled={score < 5 || saving} style={{
          ...
          {saving ? "SAVING…" : score >= 10 ? "⚡  LOG LOCK" : score >= 7 ? "✓  LOG EDGE" : score >= 5 ? "◎  LOG WATCH" : "SCORE TOO LOW"}
        </button>
...
function TodayTab({ plays, onUpdate }) {
  ...
  function setResult(id, result) { onUpdate(id, { result }); }
  function setScore(id, field, val) { onUpdate(id, { [field]: val }); }
  ...
  return (
    <div style={{ padding: "18px 20px" }}>
      <div style={{ fontSize: 11, color: "#64748b", fontWeight: 700, marginBottom: 18 }}>
            ...
            return (
              <div key={d} onClick={() => setSelectedDate(d)} style={{
                ...
                <div style={{ fontSize: 11, fontWeight: 700, color: active ? "#fff" : "#0f172a", marginBottom: 2 }}>
                  {d === TODAY_KEY ? "Today" : new Date(d + "T12:00:00").toLocaleDateString("en-US", { month: "short", day: "numeric" })}
                </div>
...
export default function MarchMadness() {
  ...
  useEffect(() => {
    dbLoadPlays()
      .then(setPlays)
      .catch(e => setError("Failed to load plays: " + e.message))
      .finally(() => setLoaded(true));
  }, []);

  async function handleAddPlay(play) {
    try {
      await dbInsertPlay(play);
      setPlays(prev => [...prev, play]);
      setTab("TODAY");
    } catch (e) {
      alert("Failed to save play: " + e.message);
    }
  }
  ...
  async function handleUpdate(id, changes) {
    ...
    if (play) {
      try {
        await dbUpdatePlay({ ...play, ...changes });
      } catch (e) {
        console.error("Failed to update play:", e);
      }
    }
  }
  ...
  if (!loaded) return (
    <div style={{ display: "flex", alignItems: "center", justifyContent: "center", height: "100vh", background: "#f1f5f9", flexDirection: "column", gap: 10 }}>
      <div style={{ fontSize: 11, color: "#64748b", fontWeight: 700, letterSpacing: 3 }}>CONNECTING TO DATABASE…</div>
    </div>
  );

  if (error) return (
    <div style={{ display: "flex", alignItems: "center", justifyContent: "center", height: "100vh", background: "#fef2f2" }}>
